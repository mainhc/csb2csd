...
local function set_basic_attr(xml_doc, data)
  ...
  local tempHori = basic_data.layoutComponent['HorizontalEdge']
  local tempVerti = basic_data.layoutComponent['VerticalEdge']

  if tempVerti~='' and tempHori ~='' then
	xml_doc:set_attribs(basic_data.layoutComponent, {
		'LeftMargin', 'RightMargin', 'TopMargin', 'BottomMargin','HorizontalEdge','VerticalEdge'
	})
  elseif tempVerti~='' and tempHori=='' then
	xml_doc:set_attribs(basic_data.layoutComponent, {
		'LeftMargin', 'RightMargin', 'TopMargin', 'BottomMargin','VerticalEdge'
	})
  elseif tempVerti=='' and tempHori~='' then	
	xml_doc:set_attribs(basic_data.layoutComponent, {
		'LeftMargin', 'RightMargin', 'TopMargin', 'BottomMargin','HorizontalEdge'
	})
  else
	xml_doc:set_attribs(basic_data.layoutComponent, {
  ...
  if basic_data.layoutComponent['StretchVerticalEnabled'] == 1 then
	child_attr(xml_doc, 'StretchHeightEnable',"True")
  end
...
local function root_node_tree(node)
  ...
  local tempName = root_option_data.Name
  local xml_doc = nil
  if tempName == "Scene" then

	xml_doc = xml.new('ObjectData', {
  ...
  child_attr(xml_doc, 'Size', { X = size.X or 0, Y = size.Y or 0 })
...
local function process(filename)
  ...
  if rc == 1 then
	...
	local csd_out_filename = csd_name
	...
	for file in lfs.dir(path) do
		if file ~= "." and file ~= ".." then
			...
			elseif 	attr.mode == "file" then				
				if string.find(f,".csb") ~= nil then				
					table.insert(allFileCsb,f)
				end											
			end
		end
	end
...
function main(Pram)
	...
	print(Pram.csbPath)	
	local filename = Pram.csbPath;
	print("++++++++++++++1")	
	attrdir(filename)
	
	print(allFileCsb[1])
	--print(table.getn(allFileCsb))
	--local iSize = table.getn(allFileCsb)
	for _,indexTemp in pairs(allFileCsb) do
	
		--print(allFileCsb[indexTemp])
		process(indexTemp)
	end
